# AirportSecuritySucks-offsets

A static IL2CPP offset parser for **Airport Security Sucks**. Takes the output of [Il2CppDumper](https://github.com/Perfare/Il2CppDumper) and generates field offsets and method RVAs in multiple formats — no injection, no runtime, just static analysis.
...
Find your Airport Security Sucks installation folder. You need two files:

| File | Path |
|------|------|
| `GameAssembly.dll` | `Airport Security Sucks/GameAssembly.dll` |
| `global-metadata.dat` | `Airport Security Sucks/Airport Security Sucks_Data/il2cpp_data/Metadata/global-metadata.dat` |
...
Airport Security Sucks is built with Unity's **IL2CPP** backend, which compiles C# game logic into a native `GameAssembly.dll`. The original C# metadata — class names, field names, offsets — is stored separately in `global-metadata.dat`. Il2CppDumper reads both files and reconstructs a C#-like `dump.cs` that annotates every field with its memory offset.
...
**Obfuscated names** — Airport Security Sucks obfuscates most method and some field names using Unicode symbols (`ↈ...ↈ`). The parser handles this gracefully:
